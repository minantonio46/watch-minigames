...
- **카드 캐러셀 메뉴**: 화면 가운데 현재 선택된 카드가 크게 표시되고 양옆에 이전/다음 카드가 미리보기 형태로 배치됩니다. 좌우 스와이프/드래그 또는 양옆 버튼 터치로 이동할 수 있으며, 키보드 방향키(`←`, `→`)도 지원합니다. (스와이프 시 게임이 오작동으로 열리지 않도록 제스처 구분이 적용되어 있습니다.)
- **즐겨찾기(★)**: 하단의 별 아이콘을 누르면 해당 게임이 즐겨찾기로 등록되어 메뉴 맨 앞쪽에 우선 정렬되며 브라우저에 저장됩니다.
- **설정 카드**: 메뉴 마지막 카드에서 설정 화면으로 이동하여 언어(한국어 / English) 및 화면 테마(다크 모드 / 라이트 모드 / 기기 설정)를 변경할 수 있습니다. 설정값은 브라우저에 저장되어 재접속 시에도 유지됩니다.
